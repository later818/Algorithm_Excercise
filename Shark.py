# 전체 알고리즘
# 1. 상어 현재 좌표 전달 시, 전체 거리를 계산 후 dis map 생성(BFS 이용)
# 2. dis map에서 size가 상어보다 작은 물고기 find(특히 최단거리인 물고기로..)
# 3. 먹을 물고기 위치와 최단거리를 main문에 반환
# 4. main은 위 내역을 기준으로 물고기 먹고 현재 상태(위치, size) 반복 갱신
# 5. 먹을 물고기가 없을 때 모든 최단거리의 합을 출력하고 시스템 stop
